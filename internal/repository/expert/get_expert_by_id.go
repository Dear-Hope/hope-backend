...
func (r *repository) GetExpertById(ctx context.Context, id uint64) (*expert.Expert, error) {
	...
	err := r.db.GetContext(ctx, &res,
		r.db.Rebind(`SELECT e.id, e.name, e.photo, e.expertise, e.price, e.is_available, e.title,
       		e.bio, e.education, e.experience, array_agg(t.name) as topic_names, AVG(er.rating) as rating
			FROM "expert".experts e, "counsel".topics t, "expert".expert_topics et, "expert".expert_reviews er
			WHERE e.id = et.expert_id AND t.id = et.topic_id AND e.id = er.expert_id AND e.id = ? GROUP BY e.id`),
		id,
	)
	if err != nil {
